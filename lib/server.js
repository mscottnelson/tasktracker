...
app.get('/crazywaytotestemail', function(req, res){
  console.log('P1: ' + req.session.username);
  console.log('P2: ' + User.findOne({ where: {
    username: req.session.username }}));
    console.log('P3: ' + User.findOne({ where: {
      username: req.session.username }}).emailKey);
  sparky.transmissions.send({
    transmissionBody: {
      content: {
        from: 'tester@' + process.env.SPARKPOST_SANDBOX_DOMAIN, // 'testing@sparkpostbox.com'
        subject: 'Hey there!',
        html: "<html><body><p>Task Tracker Account Verification.  <a href='ancient-reaches-91094.herokuapp.com/verify/" + User.findOne({ where: {
          username: req.session.username }}).emailKey + "'>Click here to confirm</a></p></body></html>"
      },
      recipients: [
