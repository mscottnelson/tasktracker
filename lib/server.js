  ...
  if (request.body.password !== request.body.password_confirm) {
    ...
    .then(function(existingUser) {
      if (existingUser) {
        ...
        models.User.create(request.body).then(function(user) {
...
app.post('/users/login', function(req, res) {
  models.User.findOne({ where: { username: req.body.user_id }})
  .then(function(user) {
    res.format({
      html: function() {
        if (!user) {
...
app.get('/verify/:user_verificationcode', function(request, response) {
  models.User.findOne({ where: { emailKey: request.params.user_verificationcode }})
  .then(function(user) {
    user.userVerified();
...
app.get('/crazywaytotestemail', function(req, res){
  console.log('P1: ' + req.session.user_id);
  models.User.findById(req.session.user_id)
    .then(function (user){
      console.log('UserKey?: ' + user.emailKey);
    });
  models.User.findById(req.session.user_id)
    .then(function (user){
      sparky.transmissions.send({
        transmissionBody: {
          content: {
            from: 'tester@' + process.env.SPARKPOST_SANDBOX_DOMAIN, // 'testing@sparkpostbox.com'
            subject: 'Hey there!',
            html: "<html><body><p>Task Tracker Account Verification.  <a href='ancient-reaches-91094.herokuapp.com/verify/" + user.emailKey + "'>Click here to confirm</a></p></body></html>"
          },
          recipients: [
            {address: 'mscottnelson@gmail.com'}
          ]
        }
      }, function(err, res) {
        if (err) {
          ...
          console.log('Woohoo! You just sent a mailing!');
        }
      });
    });
});
...
app.get('/key', function(req, res){
  console.log('P1: ' + req.session.user_id);
  console.log('n1: ' + models.User.findById(req.session.user_id)
    .then(function(user){
      console.log('n1b: ' + user.emailKey); //this is the point the I need to get to...
  }));
  console.log('P2: ' + models.User.findOne({ where: { id: req.session.user_id }}));
  console.log('P3: ' + models.User.findOne({ where: { id: req.session.user_id }}).emailKey);
  console.log('P4: ' + models.User.findOne({ }))
  res.redirect('/');
  }
);
