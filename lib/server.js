...
app.get('/crazywaytotestemail', function(req, res){
  sparky.transmissions.send({
    transmissionBody: {
      content: {
        ...
        html:"<html><body><p>Task Tracker Account Verification.  <a href='ancient-reaches-91094.herokuapp.com/verify/'" + User.findOne({ where: {
          username: request.session.username }}).emailKey + ">Click here to confirm</a></p></body></html>"
      },
      recipients: [
    ...
    if (err) {
      ...
      console.log('Woohoo! You just sent a mailing! ' + User.findOne({ where: {
        username: request.session.username }}).emailKey);
    }
  });
});
